    ...
    <div className="w-[320px] h-[480px] rounded-3xl bg-slate-950 text-white p-6 shadow-2xl relative border-4 border-slate-900/50 flex flex-col justify-between overflow-hidden select-none font-sans scale-95 origin-center">
      ...
      <div className="flex items-center gap-2 border-b border-white/10 pb-4 relative z-10">
        ...
        <div className="text-left">
          ...
          <p className="text-[10px] text-slate-400">Student Identity Card</p>
        </div>
      </div>
      ...
      <div className="flex flex-col items-center py-4 relative z-10">
        ...
        <span className="inline-flex rounded-full bg-blue-500/10 border border-blue-500/30 px-3.5 py-0.5 text-xs font-bold text-blue-400 mt-1.5">
      ...
      <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-left text-[11px] border-t border-b border-white/10 py-4 relative z-10">
        <div>
          <span className="text-[9px] uppercase tracking-wider text-slate-400 font-bold block">Student ID</span>
          <p className="font-bold text-white font-mono">{student.studentId}</p>
        </div>
        <div>
          <span className="text-[9px] uppercase tracking-wider text-slate-400 font-bold block">Admission No</span>
          <p className="font-bold text-white font-mono">{student.admissionNo}</p>
        </div>
        <div>
          <span className="text-[9px] uppercase tracking-wider text-slate-400 font-bold block">Blood Group</span>
          <p className="font-bold text-white">{student.bloodGroup || "O+"}</p>
        </div>
        <div>
          <span className="text-[9px] uppercase tracking-wider text-slate-400 font-bold block">Term Session</span>
      ...
      <div className="flex items-center justify-between pt-3 relative z-10 text-left">
        <div>
          <span className="text-[8px] uppercase tracking-wider text-slate-400 font-bold block">Emergency Call</span>
          <p className="text-xs font-bold text-slate-200">{student.fatherMobile}</p>
        </div>
